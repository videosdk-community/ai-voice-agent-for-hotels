...
This repository contains ai voice agent for hotels, built using the VideoSDK Agents architecture with a [Pipeline (Cascade Mode)](https://docs.videosdk.live/ai_agents/core-components/pipeline).

## Description

This Agent is designed to act as a helpful assistant as ai voice agent for hotels. It can answer questions and perform tasks related to this domain.
...
- A VideoSDK [API Key and Secret Key](https://app.videosdk.live?utm_source=ai-voice-agent-for-hotels&utm_medium=github-repo-auto&utm_campaign=github-repo-auto)
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/videosdk-community/ai-voice-agent-for-hotels.git
   ```

2. **Navigate to the project directory:**
   ```bash
   cd ai-voice-agent-for-hotels
   ```
...
- **`start_session` function:** This is the main asynchronous function that sets up and runs the agent.
    ...
    - `session.start(wait_for_participant=True, run_until_shutdown=True)` connects the agent to a room and starts the session.
